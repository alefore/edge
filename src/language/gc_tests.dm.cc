...
bool tests_registration = tests::Register(
    ...
    {{.name = L"CollectOnEmpty",
      .property = L"`Collect` on an empty pool returns 0 initial and final "
                  L"eden sizes.",
      .callback =
          [] {
     ...
     {.name = L"PreservesRoots",
      .property =
          L"Objects referenced by roots are not collected by `Collect`. We "
          L"validate this by asserting that the delete notification for a "
          L"rooted object is not triggered after collection.",
      .callback =
          [] {
            gc::Pool pool({});
     ...
     {.name = L"PoolRootsStartsAtZero",
      ...
      .callback = [] { /* {{🍄}} */ }},
     {.name = L"PoolRootsIncrements",
      .property = L"Creating a few roots on a pool and calling FullCollect "
                  L"returns stats with the correct count of roots.",
      .callback = [] { /* {{🍄}} */ }},
     {.name = L"PoolRootsDecrements",
      .property =
          L"Destroying a few roots (by letting them go out of scope) correctly "
          L"decrements the `roots` value in the stats returned by FullCollect.",
      .callback = [] { /* {{🍄}} */ }},
     {.name = L"BreakLoop",
      .property =
          L"Tests that a reference cycle (loop) between objects is broken and "
          L"collected when the root referencing the cycle is reassigned. It "
          L"asserts that the original objects in the loop are eventually "
          L"deleted after the root is overridden and a full collection occurs.",
      .runs = 50,
      .callback =
          [] {
            gc::Pool pool({});
     ...
     {.name = L"RootsReplaceLoop",
      .property = L"Verifies that when a root is reassigned to a new object "
                  L"loop, the old object loop is garbage collected and the new "
                  L"loop is preserved. It asserts that the total number of "
                  L"objects decreases from the sum of both loops to the size "
                  L"of the new loop after a full collection.",
      .callback =
          [] {
            gc::Pool pool({});
     ...
     {.name = L"BreakLoopHalfway",
      .property =
          L"Tests breaking a reference loop halfway by clearing the children "
          L"of a node within the loop, which should cause part of the loop to "
          L"be collected. It asserts that the object count decreases after "
          L"breaking the loop and remains consistent after a full collection, "
          L"with the remaining rooted objects preserved.",
      .callback = [] {
        gc::Pool pool({});
