...
void DoSearch(OpenBuffer& buffer, SearchOptions options) {
  ValueOrError<std::vector<LineColumn>> output =
      SearchHandler(buffer.editor().modifiers().direction, options,
...
class SearchCommand : public Command {
 ...
 private:
  static std::optional<SearchOptions> BuildPromptSearchOptions(
    ...
    if (auto limit = buffer.Read(buffer_variables::search_matches_limit);
        limit > 0)
      search_options.required_positions = limit;
