...
class Viewers {
 ...
 private:
  ...
  // returned by `Register` are still alive (because some widget hasn't detected
  // that the buffer was deleted). In that case, when the pointer returned by
  // `Register` is finally deleted, its custom deleter will detect that the
  // underlying structure is gone.
