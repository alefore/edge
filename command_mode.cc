...
class SwitchCaseTransformation : public Transformation {
 public:
  SwitchCaseTransformation(const OpenBuffer::Position& position)
      : position_(position) {}
 ...
 public:
  ...
  void ProcessInput(int c, EditorState* editor_state) {
    ...
    if (position.column == line->contents->size()) {
      ...
      buffer->set_position(OpenBuffer::Position(position.line + 1));
    } else {
      buffer->set_position(OpenBuffer::Position(position.line, position.column + 1));
    }
